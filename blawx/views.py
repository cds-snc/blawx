    ...
    if allow_registration:
        ...
        if "X-Forwarded-Email" in request.headers:
            form = UserCreationForm(
                initial={"username": request.headers["X-Forwarded-Email"]}
            )
        else:
            form = UserCreationForm()
        return render(
...
class RuleDocView(PermissionRequiredMixin, generic.DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        user = User.objects.get(username=self.kwargs["user"])
        context["tests"] = BlawxTest.objects.filter(
            ruledoc=RuleDoc.objects.get(rule_slug=self.kwargs["rule"], owner=user)
        )
...
def ruleDocLegalTextView(request, user, rule, section_name):
    ...
    if request.user.has_perm("blawx.view_ruledoc", ruledoc):
        ...
        if section_name != "root":
            ...
            return Response(
                {
                    "xml": lxml.etree.tostring(target),
...
def ruleDocExportView(request, user, rule):
    ...
    if request.user.has_perm("blawx.view_ruledoc", ruledoc):
        download = tempfile.NamedTemporaryFile(
            "w", delete=False, prefix=user + "_" + rule + "_"
        )
        download_filename = download.name
        download.write(
            serializers.serialize(
                "yaml", RuleDoc.objects.filter(rule_slug=rule, owner=owner)
            )
        )
        ...
        response = FileResponse(
            open(download_filename, "rb"),
            as_attachment=True,
            filename=user + "_" + rule + ".blawx",
        )
        return response
    else:
        return HttpResponseForbidden()
...
def exampleLoadView(request, example_name):
    if request.user.has_perm("blawx.add_ruledoc"):
        # Try both possible locations for the example file
        example_path = os.path.join(
            settings.STATIC_ROOT, "blawx", "examples", f"{example_name}.yaml"
        )
...
class TestCreateView(PermissionRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        user = User.objects.get(username=self.kwargs["user"])
        form.instance.ruledoc = RuleDoc.objects.get(
            rule_slug=self.kwargs["rule"], owner=user
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        user = User.objects.get(username=self.kwargs["user"])
        context["ruledoc_list"] = RuleDoc.objects.all()
        context["ruledoc"] = RuleDoc.objects.get(
            rule_slug=self.kwargs["rule"], owner=user
        )
        return context
...
def update_code(request, user, rule, workspace):
    ...
    if request.user.has_perm("blawx.change_workspace", target):
        ...
        target.xml_content = workspace_serializer.validated_data.get(
            "xml_content", target.xml_content
        )
        target.scasp_encoding = workspace_serializer.validated_data.get(
            "scasp_encoding", target.scasp_encoding
        )
        target.save()
...
def get_code(request, user, rule, workspace):
    ...
    if (len(target) and request.user.has_perm("blawx.view_workspace", target[0])) or (
        not len(target)
        and request.user.has_perm("blawx.add_workspace_to_ruledoc", ruledoctest)
    ):
        (workspace, created) = Workspace.objects.get_or_create(
            ruledoc=ruledoctest, workspace_name=workspace
        )
...
def duplicate_test(request, user, rule, test_name):
    owner = User.objects.get(username=user)
    target_test = BlawxTest.objects.get(
        ruledoc=RuleDoc.objects.get(rule_slug=rule, owner=owner), test_name=test_name
    )
...
def get_test(request, user, rule, test_name):
    ...
    if (len(target) and request.user.has_perm("blawx.view_blawxtest", target[0])) or (
        not len(target)
        and request.user.has_perm("blawx.add_blawxtest_to_ruldoc", ruledoctest)
    ):
        (test, created) = BlawxTest.objects.get_or_create(
            ruledoc=RuleDoc.objects.get(rule_slug=rule, owner=owner),
            test_name=test_name,
        )
